...
# TODOs
[ ] 作成時参照（テンプレートの形式） テンプレートに従って仕様書を作成する際に参照するリファレンスが必要（人間・LLM）
[ ] 作成時参照（ツールの形式） テンプレートに従って仕様書を作成する際に参照するリファレンスが必要（人間・LLM）
[ ] 実装時参照 作成された仕様書を参照して実装する際に参照するリファレンスが必要（人間・LLM）
[ ] Web版 ChatGPT と仕様を壁打ちして、最後にテンプレートを渡すと、そこに記入してくれる形式ができるとなお良い
