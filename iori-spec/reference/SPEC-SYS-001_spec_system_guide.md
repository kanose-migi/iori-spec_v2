...
- `spec_sections_registry.yaml` / `spec_sections_guide.yaml` の方針・値域・運用（例: ordering、unknown section policy、`include_in` の扱い）が変わった場合（→ SPEC-SYS-002）。
- `trace` の意味論や最小カバレッジ要件、rule ID 体系が変わり、「作法」と整合しなくなった場合（→ SPEC-SYS-003）。
- index / pack / lint_report の成果物契約や決定性要件が変わり、運用フロー（レビューや貼り方）が影響を受ける場合（→ SPEC-SYS-004）。
- ツール実行モデル、CI ゲート、exit code / profile が変わり、PR 前提が変わる場合（→ SPEC-SYS-005）。
- 仕様編集における反復的な破綻（例: 曖昧参照の再発、見出し表記ゆれの頻発、導線断絶）が観測され、チェックリストやフローの改善が必要になった場合。

### LLM 連携の原則（貼り方・渡し方）

- **全文貼りを常態化させず**、pack（局所コンテキスト）を正として運用する（局所推論の安定）。
- LLM に依頼するタスクに応じて、最小限の入力を選ぶ:
  - **新規作成/大改修**: 本仕様（SPEC-SYS-001）の該当フロー + 関連 spec の pack
...
- 仕様の差分レビューを安定させるため、見出し順・配列順・正規化は契約（SPEC-SYS-004）に寄せる。
- PR の単位で `lint`（および必要なら trace-lint）と `pack` を回し、「違反を直してから次へ進む」を基本とする（CI ゲートの具体は SPEC-SYS-005）。
