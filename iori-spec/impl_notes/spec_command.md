...
### 1. コマンド体系と iori-spec の方針の関係

背景・思想・方針の詳細は iori-spec の設計思想ドキュメント（「局所コンテキスト」「構造健全性」「スクリプトツールによる前処理」など）で説明している前提で、ここではコマンド体系との関係だけを簡潔に整理します。

- iori-spec の方針1  
  **「大規模な仕様・コードベースでも、LLM から見ると常に小さな局所だけを扱えばよい状態にする」**  
  → コマンド群は、REQ / IF / DATA / TEST / TASK を ID として扱い、  
    スクリプトベースのツールで「どの局所を LLM に渡すか」を決定する役割を持ちます。

- iori-spec の方針2  
  **「高コンテキスト前提・高額プラン前提にせず、限られたコンテキスト長と呼び出し回数の中で回せるようにする」**  
  → コマンド群は、「本当に必要な断片だけ」を機械的に切り出すことで、  
    コンテキスト長・呼び出し回数・コストの現実的な制約の中で LLM を運用することを狙います。

あわせて、依存関係・責務・**トレース（ここでは「どの REQ がどの IF/DATA/TEST でカバーされているか」という対応関係）**を明示して、
...
といった **設計品質・構造健全性** を保ちやすい仕様構造を前提にしている点も、  
コマンド体系の前提として共有しておきます。

このコマンド体系は、こうした方針を「日々触るインターフェイス」として具体化するために、  
ID・グラフ・テンプレ生成を一貫した操作体系で提供することを意図しています。

### 2. 仕様集合自身が「操作に対する影響範囲」を把握するためのインターフェイス設計

iori-spec では、仕様集合を「ノード（ID）とエッジ（依存・トレース）」からなるグラフとして扱います。

- REQ / IF / DATA / TEST / TASK は、それぞれ ID 付きのノード
- それらの関係は、主に次の2種類のエッジとして表現されます
  - **依存**：ある仕様（IF / DATA / doc など）が、別の仕様に依存している関係
  - **トレース**：ある REQ が、どの IF / DATA / TEST で満たされ・検証されているかという対応関係

この前提のもとで、

- **add / change / delete といった操作に対して、どの ID 群が影響を受けるかを自分で答えられる仕様集合**

をゴールにしています。

`index` コマンドで SSOT（インデックス）を構築し、  
`impact` や `context` で「この ID を変えたいときに、どこまでを一緒に見るべきか」をグラフから機械的に計算して返せるようにすることで、
**「仕様そのものが、自分自身の影響範囲を教えてくれる」**というインターフェイスをスクリプトベースのツールとして形にします。

### 3. LLM とツールの役割分担：構造はツール中心、意味判断は LLM

このコマンド体系では、次のような役割分担を前提にしています。

- 原則として、**構造や依存関係の把握・列挙・圧縮は、スクリプトで実装されたツール群の責務**
- その上で、**何をどう変更するか・どの案がよりよいかの判断は、LLM（と人間）の責務**

たとえば、「どの ID を読めばよいか」「どこまでを影響範囲として含めるか」を決める起点は `search` / `impact` / `context` などのコマンドであり、  
その結果として得られた局所コンテキストを受けて、仕様を設計・修正し、矛盾を検出するのが LLM の仕事です。

そのうえで、ツールが返したコンテキストだけでは判断材料が足りない場合には、  
LLM は **必要に応じて** 追加の `search` / `show` / `impact` などを実行したり、個別のファイルや ID を明示的に参照することで、
自ら情報を補完していくことを想定しています。

### 4. コマンド群の役割分類とユースケース（探索 / 影響範囲 / コンテキスト / 品質維持 / 生成）

コマンド群は、LLM が仕様と対話するときの典型的なフローに沿って、次のような役割ごとに整理されています。

- **探索（search 系）**  
  `search` で関係しそうな ID を探し、どの REQ / IF / DATA / TEST が起点になり得るかを把握する。

- **影響範囲の把握（impact 系）**  
  `impact` で変更対象の ID からグラフをたどり、「一緒に考えるべき周辺」を列挙する。

- **局所コンテキストの取得（context / show 系）**  
  `context` / `show` で、LLM に渡すための「必要最小限の局所コンテキスト」をまとめる。

- **品質維持（lint / trace-lint 系）**  
  `lint` / `trace-lint` は仕様セット全体の一貫性・トレースの欠落・構造上のアンチパターンを検出し、設計品質・構造健全性を保つ。

- **生成・スキャフォールド（new / scaffold 系）**  
  `new` / scaffold 系コマンドは、新しい REQ / IF / DATA / TEST / TASK を追加するときに、  
  型のそろった高品質な仕様テンプレートを生成する入口として機能する。

このように、それぞれのコマンドは「どの段階のユースケース（探索 / 影響範囲 / コンテキスト / 品質維持 / 生成）を支えるか」が明確になるように配置されています。
...
#### `iori-spec sections templates` — 🟢 **SHOULD**

- 役割: `spec_section_schema.yaml` と `spec_section_content.yaml` を SSOT として、kind 別の Markdown テンプレート（雛形）を生成する。
- ねらい: `iori-spec new` が利用するテンプレートと、人間がコピペして使うテンプレートを 1 ソースで維持し、「テンプレとガイドが食い違う」事態を防ぐ。
- 典型的な使い方:

  - `iori-spec sections templates --out-dir templates/spec`
  - `iori-spec sections templates --kinds requirements,interfaces --mode annotated`

#### `iori-spec sections guide` — 🟢 **SHOULD**

- 役割: 同じ 2 つの YAML から「各セクションの目的・書き方・サンプル」を統合した記載ガイド Markdown（人間向け）を自動生成する。
- ねらい: `spec_section_schema.yaml` / `spec_section_content.yaml` を編集すれば、テンプレとガイドの両方が再生成できる状態にし、セクション定義の SSOT 性を高める。
- 典型的な使い方:

  - `iori-spec sections guide --out reference/iori_spec_section_guide.generated.md`
...
- `--index <PATH>`（任意）
  ...
  - `iori-spec index` の結果として出力される index ファイルへのパス。
  - 指定がない場合、各コマンドは以下の戦略をとる：

    1. `<root>/.iori/index.json` を探す。
    2. 存在しなければ、MVP の間は内部で `index` 相当を実行してから処理を続行する（パフォーマンスは初期は妥協）。
...
- `<QUERY>`（必須・文字列）
  ...
  - 検索クエリ。自然文でもよいし、キーワード列でもよい。
  - 例: `"digits segmentation"` / `"数字列 分割 アルゴリズム"` など。

- `--kinds <KIND_LIST>`（任意・文字列）

  - 検索対象とする kind をカンマ区切りで指定する。
  - デフォルト: `requirements,interfaces,data_contracts,tests,dev_tasks`（TASK は除外してもよい）

- `--limit <N>`（任意・整数）

  - 返却する候補数の上限。デフォルトは 20 件程度を想定。
...
[
  {
    ...
    "file": "requirements/REQ-201_digits_segmentation.md",
    "score": 0.92,
    "spec_title": "digits segmentation の基本方針",
    "snippet": "digits を segments に分割するデフォルト戦略..."
  },
  {
    "id": "IF-200",
    "kind": "interfaces",
...
仕様ディレクトリ全体を走査し、**ID / kind / scope / trace / dp などを集約したインデックス**を構築する。
このインデックスは、ほぼすべてのコマンド（search / impact / context / lint / trace lint など）の共通土台になる。
...
- `--root`
  ...
  - 仕様ルートディレクトリ。省略時はカレントディレクトリ。

- `--out <PATH>`

  - 出力先パス。デフォルトは `artifacts/spec_index.json` を想定。

- `--format`

  - 現状 `json` のみ。将来的に他形式も検討余地はあるが、まずは固定でよい。
...
- `<ID>`（必須）
  ...
  - 起点となる ID（REQ-xxx / IF-xxx / DATA-xxx / TEST-xxx / TASK-xxx）。index 内で一意に存在する ID を想定。

- `--max-depth`

  - 影響グラフを辿る最大ホップ数。デフォルトは `2`。
  - `1` を指定すると、直接接続されたノードのみ。

- `--include-kinds`

  - 出力に含める kind。`req,if,data,test,task` など、どの kind を結果に含めるかをカンマ区切りで指定。
    デフォルトは全 kind。
...
指定した ID を起点に、
**LLM に渡すのに十分な「局所コンテキストパック」**を生成する。

- REQ / IF / DATA / TEST / TASK の本文から、必要なセクションだけを抜き出して 1 ファイル（あるいは 1 JSON）にまとめる。
- LLM のコンテキストにそのまま貼れる形にすることがゴール。
...
- `--radius <N>`
  ...
  - グラフ上でどこまで近傍を含めるか。`0` なら自ノードのみ、`1` なら直接つながるノードも含める。
    デフォルトは `1`。
  
- `--include-kinds`

  -  `req,if,data,test,task` など、近傍として含める kind。デフォルトは全 kind。

- `--format markdown|json`

  - `markdown`: そのまま LLM に貼れる Markdown 形式。
  - `json`: 後処理しやすい JSON 形式。
  - デフォルトは `markdown`。
...
**「LLM に渡すことを前提とした型（spec_section_schema / iori_spec_guide）」**に従っているかを検証する。
...
- `--format`
  ...
  - `text`（デフォルト）: 人間が読むことを前提としたテキストレポート。
  - `json`: 機械処理しやすい構造化レポート。
...
# TODO コマンドの設計の意図、仕様の意図を丁寧に記載したコマンド一覧が必要
# TODO コマンドの役割を軽く記載した外部向けリファレンスとしてのコマンド一覧が必要 
